            ...
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">List Buku yang Dipinjam 
             <?php if ($_SESSION['TYPE']=="SystemAdministrator" || $_SESSION['TYPE']=="Administrator") { ?>
              	<a href="index.php?view=filter" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i>Detail Buku</a>
              <?php } ?>
          	</h6>
            </div>
            <div class="card-body">
							...
							foreach ($cur as $result) {
								...
								if ($_SESSION['TYPE']=="SystemAdministrator" || $_SESSION['TYPE']=="Administrator") {
									...
									if ($result->Status=='Confirmed') {
										# code...
										$button = '<a title="Edit" href="controller.php?action=return&id='.$result->TransactionID.'" class="btn btn-primary btn-sm  " ><i class="fa fa-check"></i> Kembalikan</a>';
									}elseif($result->Status=='Pending'){
										$button = '<a title="Edit" href="controller.php?action=confirm&id='.$result->TransactionID.'" class="btn btn-primary btn-sm  " ><i class="fa fa-check"></i> Konfirmasi</a>';
									}
				  			# code...
						  			$btn= '
						  			<a title="View Book Details" href="index.php?view=view&id='.$result->TransactionID.'" class="btn btn-info btn-sm  " ><i class="fa fa-info"></i> Detail</a> 
						  			  		'.$button.'
						  					 <a title="Delete" href="controller.php?action=cancel&id='.$result->TransactionID.'&" class="btn btn-danger btn-sm  " ><i class="fa fa-times"></i> Batal</a>';
