...
switch ($action) {
   ...
   // SELECT `IDNO`, `BorrowerId`, `Firstname`, `Lastname`, `MiddleName`, `Address`, `Sex`, `ContactNo`, `CourseYear`, `BorrowerPhoto`, `BorrowerType`, `Stats`, `IMGBLOB` FROM `tblborrower` WHERE 1
	function doInsert(){
		...
		if ( $_POST['BorrowerId'] == "" OR $_POST['Firstname'] == ""
			...
			$location = "photos/". $picture ;
					...
					message("Berhasil Menambah Peminjam!", "success");
					redirect("index.php");
	...
	if(isset($_POST['save'])){
		...
		if ( $_POST['BorrowerId'] == "" OR $_POST['Firstname'] == ""
			...
			redirect('index.php?view=add');
				...
				message("Berhasil Update Informasi Peminjam!", "success"); 
	...
	function doDelete(){
		...
		// }
			message("Berhasil Menghapus Peminjam!","success");
			redirect('index.php');
		// }
  ...
  function doChangePassword(){
	...
	if(isset($_POST['save'])){
			...
			$borrower->update($_POST['BorrowerId']); 
			  ...
			  message("Berhasil mengubah password!", "success");
