            ...
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">List Buku 
             <?php if ($_SESSION['TYPE']=="SystemAdministrator" || $_SESSION['TYPE']=="Administrator") { ?>
              	<a href="index.php?view=add" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> Tambah</a>
              <?php } ?>
          	</h6>
            </div>
            <div class="card-body">
				  	...
				  	$mydb->setQuery("SELECT * FROM `tblbooks`");
				  		...
				  		function loadresult(){
							...
							$transactionCount = $mydb->loadSingleResult();
								...
								echo '<td>'. $result->BookPrice - $transactionCount->count.'</td>';
